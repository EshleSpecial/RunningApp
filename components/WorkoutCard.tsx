    ...
    >
      <Card.Content>
        <View style={styles.header}>
          ...
          {completed && (
            <Chip icon="check-circle" style={styles.doneChip} textStyle={styles.doneText}>Done</Chip>
          )}
        </View>
          ...
          <>
            ...
            {isRunType && (
              <View style={styles.accordionSection}>
                <TouchableOpacity onPress={() => setWarmupOpen(v => !v)} style={styles.accordionHeader} activeOpacity={0.7}>
                  <Text style={styles.accordionTitle}>🔥 Warmup</Text>
                  <Text style={styles.accordionChevron}>{warmupOpen ? '▲' : '▼'}</Text>
                </TouchableOpacity>
                {warmupOpen && (
                  <View style={styles.accordionBody}>
                    {WARMUP_STEPS.map((step, i) => <Text key={i} style={styles.accordionItem}>• {step}</Text>)}
                  </View>
                )}
              </View>
            )}
            ...
            {isRunType && (
              <View style={styles.accordionSection}>
                <TouchableOpacity onPress={() => setCooldownOpen(v => !v)} style={styles.accordionHeader} activeOpacity={0.7}>
                ...
                {cooldownOpen && (
                  <View style={styles.accordionBody}>
                    ...
                    {COOLDOWN_STEPS.map((step, i) => <Text key={i} style={styles.accordionItem}>• {step}</Text>)}
                    <Text style={[styles.accordionSubheader, { marginTop: 10 }]}>Foam Roll</Text>
                    {ROLLOUT_STEPS.map((step, i) => <Text key={i} style={styles.accordionItem}>• {step}</Text>)}
                  </View>
                )}
              </View>
            )}
            ...
            {completed && isRunType && onSaveRunData && (
              <PostRunLogger workoutDate={workout.date} logEntry={logEntry} onSave={onSaveRunData} />
            )}
          </>
        )}
      </Card.Content>
      ...
      {!compact && !completed && onComplete && (
        <Card.Actions>
          {showSwap && onSwapCrossTraining && workout.type !== 'rest' && workout.type !== 'pt_only' && (
            <Button mode="outlined" onPress={onSwapCrossTraining} style={styles.swapBtn} labelStyle={{ fontSize: 12 }}>
              Swap to Cross-Train
            </Button>
          )}
          <Button mode="contained" onPress={onComplete} style={{ backgroundColor: config.color }}>Mark Complete</Button>
        </Card.Actions>
      )}
    </Card>
  );
...
const styles = StyleSheet.create({
  card: { marginVertical: 6, borderLeftWidth: 5, borderRadius: 10 },
  header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' },
  titleRow: { flexDirection: 'row', alignItems: 'center', gap: 8, flex: 1 },
  emoji: { fontSize: 22, marginRight: 4 },
  subtitle: { color: '#6b7280', marginTop: 1 },
  notes: { marginTop: 8, color: '#374151', lineHeight: 18 },
  treadmillBox: { marginTop: 8, backgroundColor: '#f0fdf4', borderRadius: 8, paddingHorizontal: 10, paddingVertical: 6 },
  treadmillText: { color: '#15803d', fontSize: 13, fontWeight: '600' },
  fuelingBox: { marginTop: 8, backgroundColor: '#fffbeb', borderRadius: 8, paddingHorizontal: 10, paddingVertical: 8, borderLeftWidth: 3, borderLeftColor: '#f59e0b' },
  fuelingTitle: { color: '#92400e', fontSize: 13, fontWeight: '700', marginBottom: 3 },
  fuelingDetail: { color: '#78350f', fontSize: 12, lineHeight: 17 },
  accordionSection: { marginTop: 8, borderRadius: 8, overflow: 'hidden', borderWidth: 1, borderColor: '#e5e7eb' },
  accordionHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 12, paddingVertical: 9, backgroundColor: '#f9fafb' },
  accordionTitle: { fontSize: 13, fontWeight: '600', color: '#374151' },
  accordionChevron: { fontSize: 10, color: '#9ca3af' },
  accordionBody: { paddingHorizontal: 12, paddingVertical: 10, backgroundColor: '#fff' },
  accordionSubheader: { fontSize: 11, fontWeight: '700', color: '#6b7280', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 },
  accordionItem: { fontSize: 12, color: '#374151', lineHeight: 20 },
  doneChip: { backgroundColor: '#d1fae5' },
  doneText: { color: '#065f46', fontSize: 11 },
  swapBtn: { borderColor: '#d97706' },
});
