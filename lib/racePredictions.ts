...
/**
 * Builds race time predictions using the Riegel formula.
 * Prefers actual race-day logged paces; falls back to longest training run;
 * falls back to the user's profile easy pace with a ~12% effort adjustment.
 */
export function buildPredictions(
  ...
  let baseMiles = 10;
  let basePace = easyPaceMinPerMile * 0.88; // easy → race pace estimate
  ...
  if (log && plan) {
    ...
    if (raceRuns.length > 0) {
      ...
      basePace = raceRuns[0].pace;   // actual race pace — use directly
      basedOnActual = true;
    } else if (trainRuns.length > 0) {
      baseMiles = trainRuns[0].miles;
      basePace = trainRuns[0].pace * 0.88; // training easy → estimated race effort
      basedOnActual = true;
    }
  }
...
/** Returns recent pace log entries sorted newest first. */
